...
def getbeta(optc_grgf, optc_sgr, optc_sgf):
    return (optc_grgf + optc_sgr - optc_sgf) / 2
...
class DeceptiveSearchAgentpi2(SearchAgent):
    ...
    def registerInitialState(self, state):
        ...
        goalReal = state.getFood().asList()[0]
        goalFalse = state.getCapsules()[0]
        pacState = state.getPacmanPosition()
        newProblem = self.searchType(state)
        aStarSearch = getattr(search, "aStarSearch_task4_1")

        optc_sgr = len(aStarSearch(newProblem, pacState, goalReal, heuristic=nullHeuristic)[1])
        optc_sgf = len(aStarSearch(newProblem, pacState, goalFalse, heuristic=nullHeuristic)[1])
        optc_grgf = len(aStarSearch(newProblem, goalReal, goalFalse, heuristic=nullHeuristic)[1])
        beta = getbeta(optc_grgf, optc_sgr, optc_sgf)
        statepath_grgf = (aStarSearch(newProblem, goalReal, goalFalse, heuristic=nullHeuristic)[0])[3]
        for index, statepath in enumerate(statepath_grgf):
            if index >= beta:
                target = statepath[0]
                break

        self.actions = aStarSearch(newProblem, pacState, target, heuristic=nullHeuristic)[1] + aStarSearch(newProblem, target, goalReal, heuristic=nullHeuristic)[1]
        return self.actions
...
class DeceptiveSearchAgentpi3(SearchAgent):
    ...
    def registerInitialState(self, state):
        ...
        newProblem = self.searchType(state)
        aStarSearch = getattr(search, 'aStarSearch_task4_1')
        aStarSearchDecep = getattr(search, "aStarSearch_task4_2")
        ...
        for i, statepath in enumerate(statepath_grgf):
            if i >= beta:
                ...
                break
        path_st = \
            aStarSearchDecep(newProblem, pacState, target, goalReal, goalFalse, alpha, heuristic=manhattanHeuristic1)[1]
        path_tgr = aStarSearch(newProblem, target, goalReal, heuristic=nullHeuristic)[1]
...
def shortDistance(food, capsules, problem):
    queue = util.Queue()
    item = (food, 0)
    queue.push(item)
    problem.heuristicInfo['dst'] = {food: 0}
    while not queue.isEmpty():
        pos, cost = queue.pop()
        x, y = pos
        if not problem.walls[x - 1][y]:
            next = (x - 1, y)
            add_cost = 0
            if next not in capsules:
                add_cost = 1
            next_cost = cost + add_cost
            if next not in problem.heuristicInfo['dst'] or problem.heuristicInfo['dst'][next] > next_cost:
                problem.heuristicInfo['dst'][next] = next_cost
                add_item = (next, next_cost)
                queue.push(add_item)

        if not problem.walls[x][y - 1]:
            next = (x, y - 1)
            add_cost = 0
            if next not in capsules:
                add_cost = 1
...
def foodHeuristic(state, problem):
    ...
    max_food_distance = None
    for food in food_list:
        shortDistance(food, capsules, problem)
        for another in food_list:
            distance = problem.heuristicInfo['dst'][another]
            if max_food_distance == None or max_food_distance[2] < distance:
                max_food_distance = (food, another, distance)

    shortDistance(position, capsules, problem)
    d1 = problem.heuristicInfo['dst'][max_food_distance[0]]
    d2 = problem.heuristicInfo['dst'][max_food_distance[1]]
    hcost = max_food_distance[2] + min(d1, d2)
    return hcost
